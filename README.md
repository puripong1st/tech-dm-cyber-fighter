...
| `questions_bank.html` | คลังข้อสอบและเฉลย 75 ข้อ พร้อมระบบกรองตามด่าน/Bloom's Taxonomy และการยืนยันสิทธิ์แอดมินปลดล็อกเฉลย |
...
*   **ระบบ HP (Hit Points) และ AP (Action Points):**
    *   ผู้เล่นและบอสแต่ละด่านจะมีคะแนนพลังชีวิต (HP) สูงสุด 100 HP
    *   เมื่อผู้เรียนตอบถูก: จะคำนวณการโจมตีสร้างความเสียหายต่อบอส โดย HP ของบอสจะลดลงตามสัดส่วนของจำนวนคำถามในด่าน เพื่อให้ผู้เรียนได้ตอบคำถามครบทุกข้อในด่าน (Stage 1: 3 ข้อ, Stage 2: 4 ข้อ, Stage 3: 4 ข้อ, Stage 4: 4 ข้อ, Stage 5: 10 ข้อ รวมทั้งหมด 25 ข้อ)
    *   เมื่อตอบถูก จะได้รับค่า AP สะสม 10 หน่วย (หรือ 5 หน่วยหากตอบผิดในรอบแรก) สำหรับใช้สกิลช่วยเหลือ เช่น โล่ป้องกัน (50 AP), ฟื้นฟู HP (60 AP), หรือ 50/50 ตัดตัวเลือก (75 AP)
*   **ระบบคอมโบสะสม (Combo Bonus):**
    *   เก็บสถิติจำนวนการตอบถูกติดต่อกันในตัวแปร JavaScript `comboCount`
    *   หากตอบถูกต่อเนื่อง ตัวคูณ DAMAGE Multiplier จะเพิ่มขึ้นเรื่อย ๆ (สูงสุด 2.0x) พร้อมแสดงผลสถานะคอมโบ (`X COMBO!`) เพื่อสร้างความท้าทายและสีสันในการต่อสู้ โดยการเปลี่ยนด่านจะเกิดขึ้นเมื่อผู้เรียนตอบคำถามครบทุกข้อในด่านนั้น (เพื่อป้องกันการข้ามคำถามและเพื่อให้สะสมคะแนนเต็ม 5,000 คะแนนได้อย่างถูกต้อง) หากตอบผิด คะแนนคอมโบจะถูกล้างค่ากลับเป็น 0

#### C. ระบบคิดคะแนนรวม 5,000 คะแนน และคะแนนเก็บเต็ม 10 (Scoring System)

ระบบคะแนนของ Cyber Fighter ใช้คะแนนหลักเต็ม **5,000 คะแนน** เป็นค่าที่บันทึกลงฐานข้อมูล `cyber_fighter_scores.score` และเป็นคะแนนที่หน้า Teacher Dashboard แสดงบนเว็บบอร์ด ส่วนคะแนนเต็ม 10 เป็นคะแนนแปลงสำหรับใช้ในรายงาน PDF ของครู

##### 1. สูตรคะแนนหลัก

คะแนนรวมสุดท้ายคำนวณจาก 3 ส่วน:

```text
คะแนนรวม = คะแนนคำตอบ + คะแนน HP คงเหลือ - คะแนนหักโทษ
```

ในโค้ดใช้สูตร:

```javascript
finalScore = Math.min(5000, Math.max(0, answerScore + hpBonus - penalty));
```

ความหมายคือ:

* คะแนนจะไม่เกิน 5,000 ต่อให้มีเงื่อนไขพิเศษใด ๆ ทำให้ผลรวมสูงกว่า 5,000
* คะแนนจะไม่ต่ำกว่า 0 ต่อให้เกมโอเวอร์หรือผิดเยอะมาก
* คะแนนเต็ม 5,000 จะเกิดเมื่อผู้เรียนทำข้อครบ, ตอบถูกครบตามระบบ, HP สุดท้ายเหลือ 100, และไม่มีข้อผิดที่ถูกนับโทษ

##### 2. คะแนนคำตอบเต็ม 4,500 คะแนน

คะแนนส่วนคำตอบคิดจากจำนวนข้อที่ตอบถูกจริงในรอบนั้น

```text
คะแนนคำตอบ = round((จำนวนข้อที่ตอบถูก / จำนวนข้อทั้งหมด) × 4,500)
```

รอบปกติของเกมใช้คำถามทั้งหมด **25 ข้อ**:
...
ดังนั้นถ้าเล่นครบ 25 ข้อ คำตอบถูกแต่ละข้อมีค่าน้ำหนักประมาณ:

```text
4,500 / 25 = 180 คะแนนต่อข้อ
```

ตัวอย่างคะแนนคำตอบ:

| จำนวนข้อที่ตอบถูก | คะแนนคำตอบโดยประมาณ |
|---:|---:|
| 25/25 | 4,500 |
| 24/25 | 4,320 |
| 23/25 | 4,140 |
...
ข้อสำคัญ: เกมนี้เมื่อผู้เรียนตอบผิด ระบบจะให้ดูเฉลยแล้วบังคับกลับมาทำข้อเดิมซ้ำจนกว่าจะตอบถูกก่อนจึงไปข้อต่อไปได้ ดังนั้นถ้าผู้เรียนเล่นจนจบเกมครบทุกข้อ แม้เคยตอบผิดระหว่างทาง คะแนนคำตอบส่วน 4,500 ยังมีโอกาสเต็ม เพราะสุดท้ายทุกข้อถูกตอบถูกแล้ว แต่ข้อผิดจะไปกระทบคะแนนผ่านระบบ HP และค่าปรับแทน

##### 3. คะแนน HP คงเหลือเต็ม 500 คะแนน

คะแนน HP คิดจากเลือดผู้เล่นที่เหลืออยู่ตอนจบเกม

```text
คะแนน HP = HP คงเหลือ × 5
```
...
โดยปกติเมื่อตอบผิดและไม่มีโล่ป้องกัน ผู้เล่นจะเสีย HP 10 หน่วย ซึ่งเท่ากับเสียคะแนน HP 50 คะแนน แต่ระหว่างผ่านด่าน ระบบมีการฟื้น HP หลังจบด่าน ทำให้ผลเสียจาก HP อาจถูกชดเชยบางส่วนหรือทั้งหมดได้ ขึ้นอยู่กับว่าพลาดช่วงไหนและหลังจากนั้นมีการฟื้น HP กลับมาเท่าไร

##### 4. คะแนนหักโทษจากข้อผิด

เมื่อตอบผิดและไม่ได้ถูกโล่ป้องกัน ระบบจะนับข้อผิดใน `currentSessionMistakes`

```text
คะแนนหักโทษ = จำนวนครั้งที่ตอบผิด × 50
```

ตัวอย่าง:

| จำนวนครั้งที่ตอบผิด | คะแนนหักโทษ |
|---:|---:|
| 0 | 0 |
| 1 | 50 |
| 2 | 100 |
...
ดังนั้นการตอบผิด 1 ครั้งแบบไม่มีโล่ จะมีผลอย่างน้อยคือโดนหักโทษ 50 คะแนน และอาจเสียคะแนน HP อีก 50 คะแนนหาก HP ที่เสียไปไม่ได้ถูกฟื้นกลับมาก่อนจบเกม

##### 5. ตารางตัวอย่างคะแนนเมื่อเล่นจบครบทุกข้อ

ตารางนี้สมมติว่าเล่นจบครบ 25 ข้อ และสุดท้ายทุกข้อถูกตอบถูกแล้ว คะแนนคำตอบจึงเต็ม 4,500 คะแนน ความต่างของคะแนนเกิดจากจำนวนข้อผิดและ HP ตอนจบ

| ผิดกี่ครั้ง | HP ตอนจบ 100 | HP ตอนจบ 90 | HP ตอนจบ 80 | HP ตอนจบ 70 | HP ตอนจบ 60 |
|---:|---:|---:|---:|---:|---:|
| 0 | 5,000 | 4,950 | 4,900 | 4,850 | 4,800 |
| 1 | 4,950 | 4,900 | 4,850 | 4,800 | 4,750 |
| 2 | 4,900 | 4,850 | 4,800 | 4,750 | 4,700 |
| 3 | 4,850 | 4,800 | 4,750 | 4,700 | 4,650 |
| 4 | 4,800 | 4,750 | 4,700 | 4,650 | 4,600 |
| 5 | 4,750 | 4,700 | 4,650 | 4,600 | 4,550 |
| 10 | 4,500 | 4,450 | 4,400 | 4,350 | 4,300 |

อ่านตารางแบบนี้:

* ถ้าผิด 0 ครั้ง และ HP ตอนจบ 100 จะได้ 5,000
* ถ้าผิด 1 ครั้ง แต่ HP ถูกฟื้นกลับมาเต็ม 100 ก่อนจบ จะได้ 4,950
* ถ้าผิด 1 ครั้ง และ HP ตอนจบเหลือ 90 จะได้ 4,900
* ถ้าผิด 5 ครั้ง และ HP ตอนจบเหลือ 60 จะได้ 4,550
* ถ้าผิดเท่ากัน แต่ HP ตอนจบต่างกัน คะแนนจะต่างกันตาม HP ที่เหลือ เพราะ HP 1 หน่วยเท่ากับ 5 คะแนน

##### 6. กรณีเกมโอเวอร์ก่อนจบครบทุกข้อ

ถ้าผู้เรียน HP หมดก่อนจบเกม คะแนนคำตอบจะคิดจากจำนวนข้อที่ตอบถูกจริงจนถึงจุดนั้น ไม่ใช่ 4,500 เต็ม

ตัวอย่าง ถ้ารอบนั้นมีทั้งหมด 25 ข้อ แต่ตอบถูกจริงก่อนเกมโอเวอร์ 12 ข้อ:

```text
คะแนนคำตอบ = round((12 / 25) × 4,500) = 2,160
```

ถ้า HP ตอนจบเป็น 0 และผิด 10 ครั้ง:

```text
คะแนนรวม = 2,160 + (0 × 5) - (10 × 50)
คะแนนรวม = 2,160 - 500
คะแนนรวม = 1,660
```

##### 7. ผลของสกิลโล่ป้องกันต่อคะแนน

สกิลโล่มีผลสำคัญต่อการคิดคะแนน:

* ถ้า `shieldActive` ทำงานอยู่และผู้เรียนตอบผิด ระบบจะบล็อกความเสียหาย
* เมื่อตอบผิดแล้วถูกโล่บล็อก ระบบปัจจุบันจะไม่เพิ่ม `currentSessionMistakes`
* หมายความว่าการตอบผิดครั้งนั้นไม่เสีย HP และไม่โดนหักโทษ 50 คะแนน
* แต่ผู้เรียนยังต้องกลับมาตอบข้อเดิมให้ถูกก่อนจึงไปข้อต่อไปได้

ดังนั้นโล่ช่วยป้องกันคะแนนตกได้จริง เพราะกันทั้งความเสียหาย HP และการนับข้อผิดในระบบคะแนน

##### 8. คะแนนเต็ม 10 สำหรับรายงาน PDF

คะแนนเต็ม 10 ไม่ใช่คะแนนที่ใช้แสดงบนเว็บบอร์ดหลัก แต่เป็นคะแนนที่แปลงจากคะแนนรวม 5,000 เพื่อให้ครูนำไปใช้เป็นคะแนนเก็บได้ง่ายในรายงาน PDF

สูตรคือ:

```text
คะแนนเต็ม 10 = ((คะแนนรวม / 5,000) × 10)
```

ใน Teacher Dashboard ตอนส่งออก PDF ระบบแสดงเป็นทศนิยม 1 ตำแหน่งด้วย `.toFixed(1)`

ตัวอย่าง:

| คะแนนรวม 5,000 | คะแนนใน PDF เต็ม 10 |
|---:|---:|
| 5,000 | 10.0 / 10 |
| 4,950 | 9.9 / 10 |
| 4,900 | 9.8 / 10 |
...
สรุปสั้น ๆ คือ หน้าเว็บบอร์ดครูแสดงเฉพาะคะแนนรวม 5,000 เพื่อให้ตารางอ่านง่าย ส่วนไฟล์ PDF แสดงทั้งคะแนนรวมและคะแนนเต็ม 10 เพื่อใช้ส่งออกหรือกรอกคะแนนเก็บ
...
*   **การจัดเก็บลง Supabase Table:** เมื่อจบการต่อสู้ด่านที่ 5 ระบบจะส่งคำขอเขียนตารางฐานข้อมูลโดยสั่งสืบค้นและอัปโหลดคะแนนของนักเรียนเข้าคลาวด์ฐานข้อมูลแบบเรียลไทม์:
    ...
    *   ข้อมูลที่ส่งขึ้นคลาวด์: ชื่อจริง (`name`), นามสกุล (`surname`), เพศ (`gender`), เลขที่ (`student_number`), ห้องเรียน (`room`), คะแนนที่ทำได้จริงสูงสุด (`score`), และจำนวนครั้งที่พยายามทดสอบสะสม (`attempts`)
...
*   **ระบบคัดกรองตัวกรองและจัดเรียงแบบไดนามิก (Dynamic Filters & Sorting):**
    ...
    *   *ระบบกรองแยกห้องเรียนและคัดแยกเพศ:* เมนูดรอปดาวน์สำหรับคัดกรองคะแนนนักเรียนตามห้องเรียน (ม.3/1 - ม.3/15) ร่วมกับเมนูดรอปดาวน์คัดแยกเฉพาะเพศชาย (ชาย ♂) หรือเพศหญิง (หญิง ♀) เพื่อเอื้ออำนวยความสะดวกในการจัดกลุ่มให้ครูคัดแยกรายชื่อและคะแนนกรอกลงใบรายชื่อกระดาษได้ง่ายและรวดเร็ว
    *   *ระบบจัดเรียงคะแนนเก็บ:* คุณครูสามารถเลือกจัดเรียงตามเลขที่ (น้อย ➔ มาก), คะแนนเก็บสูงสุด ➔ ต่ำสุด, เวลาเล่น หรือเรียงตามชื่อเพื่อตรวจเช็คได้ง่ายขึ้น
*   **ระบบสร้างรายงานอิเล็กทรอนิกส์ (html2pdf.js integration):**
    *   บูรณาการไลบรารีภายนอกเพื่อสร้างรายงาน
    *   เมื่อคุณครูกดปุ่ม "ส่งออกเป็น PDF (Export Report)" ฟังก์ชันจาวาสคริปต์จะจัดรูปแบบหัวเอกสาร ใส่ชื่อรายวิชา และดึงข้อมูลตารางคะแนนตามเงื่อนไขตัวกรองมาส่งออกเป็นไฟล์เอกสาร PDF ขนาดกระดาษ A4 แนวตั้งอัตโนมัติ โดยปรับเปลี่ยนดีไซน์ตารางให้อ่านและมองง่าย (เช่น การสลับสีแถวเพื่อเป็นแนวสายตา, การเน้นความเด่นชัดของคอลัมน์คะแนนเก็บเต็ม 10 ด้วยสีพื้นหลังไฮไลต์ และสีจำแนกเพศอย่างเด่นชัด) เพื่ออำนวยความสะดวกให้ครูตรวจสอบและกรอกคะแนนได้ทันทีโดยไม่สับสนแถว
...
### 7. คลังข้อสอบ & เฉลยสำหรับครูผู้สอน (`questions_bank.html`)
เป็นหน้ารวมโจทย์คำถาม ตัวเลือก และคำอธิบายเฉลยทั้งหมด 75 ข้อในระบบ เพื่ออำนวยความสะดวกให้คุณครูอ่านตรวจทาน ทบทวนเนื้อหา หรือสั่งพิมพ์เป็นเอกสาร
*   **การดึงข้อมูลจาก dynamic storage (`game_questions.js`):** อ่านข้อมูล `QUESTION_BANK`, `STAGE_CONFIG` และ `BLOOM_LEVELS` มาสร้างเป็น Question Cards แบบไดนามิกโดยอัตโนมัติ
*   **ระบบคัดกรองและค้นหาคำถาม (Multi-filtering & Real-time Search):**
    *   สามารถเลือกกรองข้อสอบเฉพาะด่าน (Stage 1 ถึง Stage 5) หรือเฉพาะระดับพฤติกรรมการเรียนรู้ (การจำ / การเข้าใจ / การประยุกต์ใช้)
    *   ช่องค้นหา (Search input) ทำงานแบบเรียลไทม์ ตรวจจับคำสำคัญทั้งในโจทย์ ตัวเลือก คำตอบ หรือคำอธิบายเฉลย
*   **ระบบล็อกและปลดล็อกเฉลยด้วยสิทธิ์แอดมิน (Admin Protection & Passcode Modal):**
    *   โดย default เฉลยและคำอธิบายจะถูกซ่อนไว้เพื่อป้องกันผู้เรียนแอบเข้ามาดูเฉลย
    *   มี Modal ยืนยันสิทธิ์แอดมินเมื่อผู้ใช้กดปุ่ม "เข้าสู่ระบบแอดมิน" โดยระบบจะตรวจสอบรหัสผ่านกับ `TEACHER_PASSCODE` ที่ถูกฉีดมาจากไฟล์ `.env` ในขั้นตอนบิลด์
    *   เมื่อยืนยันรหัสถูกต้อง ระบบจะไฮไลต์คำตอบที่ถูกต้องด้วยสีเขียวนีออน (`✓`) และเปิดแสดงกล่องคำอธิบายเฉลยทันที พร้อมบันทึกสถานะล็อกอินไว้ใน `sessionStorage`
*   **ระบบจัดหน้าสำหรับพิมพ์เอกสาร (`@media print`):** ปรับเปลี่ยนธีมเป็นพื้นหลังสีขาว ข้อความสีดำ และซ่อนเมนูที่ไม่จำเป็นเมื่อสั่งพิมพ์ผ่านเบราว์เซอร์ เพื่อให้พิมพ์ใส่กระดาษ A4 ได้อย่างสะอาด คลีน และอ่านง่าย
...
sequenceDiagram
    ...
    Student->>Game: กรอกชื่อ นามสกุล เลขที่ ห้อง และเลือกเพศ (ชาย/หญิง)
